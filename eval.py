...
if __name__ == "__main__":
    ...
    # Calculate Dataset-Level Raw Averages
    dataset_ooc_rate = (global_ooc_count / global_char_lines * 100) if global_char_lines > 0 else 0.0
    dataset_logic_rate = (global_violated_count / global_logic_events * 100) if global_logic_events > 0 else 0.0
    dataset_avg_conflict = cumulative_conflict_score / len(script_chunks) if script_chunks else 0.0
    
    # ---------------------------------------------------------
    # Apply DramaBench 0-100 Normalization Math
    # ---------------------------------------------------------
    db_character_score = 100.0 - dataset_ooc_rate
    db_logic_score = 100.0 - dataset_logic_rate
    # Min-Max Scaling for Conflict: (Score - Min) / (Max - Min) * 100 -> (Score - (-5)) / (2 - (-5)) * 100
    db_conflict_score = ((dataset_avg_conflict + 5.0) / 7.0) * 100.0
    
    db_3dim_average = (db_character_score + db_logic_score + db_conflict_score) / 3.0
    ...
    with open(csv_raw_filename, mode='w', newline='', encoding='utf-8') as f:
        ...
        for record in chunk_wise_records:
            writer.writerow([
                TRANSCRIPT_PATH, 
                record["chunk_id"], 
                record["character_ooc_rate_percent"], 
                record["logic_break_rate_percent"], 
    ...
    with open(csv_leaderboard_filename, mode='w', newline='', encoding='utf-8') as f:
        writer = csv.writer(f)
        writer.writerow(["Model / Architecture", "Character Consistency (CC)", "Logic Consistency (LC)", "Conflict Handling (CH)", "3-Dim Average Score"])
        
        # Hardcoding the model name for this run, but you can parameterize this later
        writer.writerow([
            "Agentworld Baseline (Blind Physics)", 
            round(db_character_score, 2), 
            round(db_logic_score, 2), 
            round(db_conflict_score, 2), 
            round(db_3dim_average, 2)
        ])
